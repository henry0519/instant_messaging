...
class WebSocketEvent
{
    ...
    public function onClose(\swoole_server $server, int $fd, int $reactorId)
    {
        ...
        if ($info && $info['websocket_status'] === WEBSOCKET_STATUS_FRAME) {
            ...
            //账户下线--暂时不做操作
            $get_id = $redis->HGET('Client-User-relationship',$fd);
            $lawyer = $redis->HEXISTS('Lawyer',$get_id.'_lawyer');
            $user = $redis->HEXISTS('User','user_'.$get_id);
            if($lawyer){
                $table = 'Lawyer';
                $column = $get_id.'_lawyer';
            }
            if($user){
                $table = 'User';
                $column = 'user_'.$get_id;
            ...
            if((isset($table) && isset($column))){
                ...
                $redis->HSET($table,$column,json_encode($userinfo));
                
                $C_U_ID = $redis->HGET('Client-User-relationship',$fd);
                $redis->HDEL('User-Client-relationship',$C_U_ID);
                $redis->HDEL('Client-User-relationship',$fd);
            }
            /**
             * 判断连接是否是 server 主动关闭
